#### PROPOSED CHANGES TO "test_session4.py" :) ####
#### Each change indicted with "CHANGE <change_no>" ####
...
#         # changing " == 10" to " <= 10": 
#         # 1. To handle float values that can be represented perfectly in binary
#         #       e.g., 0.01171875 -> 0.00000011 in binary has exactly 8 decimal places 
#         # 2. To handle float values that may have a trailing zero when rounded off 
#         #       e.g., 0.12345678901 -> 0.123456789 (has nine decimal places) when rounded off using round(value, 10) 
...
#     # changing ">" to ">=" :
#     # as random.uniform(-1,1) can generate a value equal to -1 (though a very rare case and almost has zero probability of occurance :P)

#     # changing "<" to "<=" :
#     # though random.uniform(-1,1) can never generate a value equal to 1,
#     # in a rare case the value can be equal to one as we use round(value, 10) to round it off to 10 decimal places
#     # say, if value generated is, 0.99999999999 then round(value, 10) == 1 (though a very rare case and almost has zero probability of occurance :P)
...
#     # changing q1 = session4.Qualean() to q1 = session4.Qualean(1) as q1 can be 0 and test fails even if q2 None type is handled
#     #q1 = session4.Qualean()
